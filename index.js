...
app.get("/random", (req, res) => {
  res.json(jokes[Math.floor(Math.random() * 100) + 1]);
})

//2. GET a specific joke
app.get("/jokes/:id", (req, res) => {
  var id = parseInt(req.params.id);
  var find = jokes.find((joke) => joke.id === id);
  res.json(find);
})
...
app.get("/filter", (req, res) => {
  ...
  var find = jokes.filter((joke) => joke.jokeType === type);
  res.json(find);
})
...
app.delete("/all", (req, res) => {
  ...
  if (userKey === masterKey) {
    ...
    res
      .status(404)
      .json({ error: `You are not authoristed to perform this action` });
  }
})
